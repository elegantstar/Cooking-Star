...
**크기가 큰 이미지 파일을 서버에서 받아 클라이언트에 재전달 하는 과정에서 통신 부하가 크게 발생하였고, 성능 저하를 야기했습니다.** 따라서 클라이언트에서 직접 파일을 요청하도록 변경하는 것으로 응답 속도를 크게 개선할 수 있었습니다. 기존 방식은 이미지 파일 10건 조회 시 평균 응답 속도가 **400ms** 내외로 사용자가 크게 불편을 느낄만한 수준이었으나, 변경 이후에는 **평균 58ms**으로 **기존 대비 85% 이상 향상**되었습니다.

| 이미지 파일 전송 방식                  | 응답 속도(10건 조회 시 평균값) |
| -------------------------------------- | ------------------------------ |
| 서버를 거쳐 클라이언트에 전달하는 방식 | 400ms 이상                     |
| 클라이언트에서 S3에 직접 요청하는 방식 | 58ms                           |
