...
<img src=https://raw.githubusercontent.com/elegantstar/SocialMedia-like-Instagram/feature/entity-deletion-bugfix/images/erd.png>
...
이미지 저장소는 1차 개발 단계에서 `로컬 디스크`를 사용하는 방식으로 구현하였으나, 이후 `AWS S3`를 사용하는 것으로 리팩토링을 진행하였습니다. 이미지 업로드는 `비동기`로 이루어지며, 아래와 같은 프로토콜을 갖습니다.

1. 클라이언트가 서버에 이미지 파일을 전송합니다.
2. 서버는 클라이언트로부터 받은 이미지 파일명을 별도의 알고리즘을 통해 `고유한 이름`으로 변경하여 S3에 저장하고, 변경된 고유한 파일명을 이미지의 `URI`로써 클라이언트에 전달합니다.
3. 클라이언트는 전달 받은 `URL`을 통해 이미지 파일을 요청합니다.
4. 서버는 `URL`에 해당하는 이미지 파일을 `AWS Cloudfront`에 요청하고, 전달 받은 이미지 파일을 클라이언트에 전달합니다.

#### 이미지 파일 저장소를 로컬 디스크에서 AWS S3로 변경한 이유

`안정성`, `확장성`, `서비스 배포`를 고려하여 변경하였습니다. 서버를 띄우는 로컬 디스크에 파일을 저장하는 것은 응답속도 면에서 매우 빠르지만, **파일의 안정적인 관리 측면에서는 많은 리스크가 존재하기에 원격 저장소가 필요하다고 판단**했습니다. 또한, 추후 서비스를 배포하는 것을 염두에 두고 개발을 진행하였기 때문에 **서비스의 안정적인 운영** 측면에서도 필요한 선택이었습니다.
...
#### 원격 서버 사용으로 인한 응답 속도 저하 이슈

파일의 저장소를 로컬에서 S3로 변경하면서 이미지 파일 요청에 대한 응답 속도가 현저히 저하되는 이슈가 발생하였습니다. 이 문제를 해결하기 위해 두 가지 방법을 적용하였습니다.

##TODO: 이미지 파일 불러오는 방식 변경 필요!!!!!!

1. 서버를 거쳐서 파일을 전송할 경우 느린 응답 속도
2. 대역폭 이슈 -> 여러 명의 유저가 이미지 파일을 요청하는 경우 대역폭 터져서 인터넷 안 됨.

##### AWS Cloudfront

속도 저하를 해결하려던 중에 AWS에서 제공하는 `CDN 서비스(Cloudfront)`가 있다는 것을 알게 되었습니다.  
이미 S3의 버킷을 `Region`을 '아시아 태평양(서울) ap-northeast-2'에 만들었으나, Cloudfront를 사용하면 약간이라도 성능적 이점이 있을 것으로 판단하고 적용하였습니다. Cloudfront에서도 S3에 대한 원본 파일을 `Caching`하는 것이 가능했고, 현재 프로젝트 정책 상 이미지에 대한 수정은 허용하지 않기에 Cache의 기간을 충분히 크게 설정하여 응답 속도를 개선하였습니다.

##### Browser Cache

Cloudfront 연동에도 불구하고 여전히 응답 속도에 대한 아쉬운 결과를 보였습니다. 이에 따라 이미지 파일에 대해 `브라우저 캐시`를 적용하여 사용자의 불편함을 해소하였습니다.
...
Mybatis를 사용하여 개발하는 것 자체에는 아무런 문제가 없었으나 몇 가지 불편함을 느꼈습니다. 그중 가장 크게 느낀 불편함은 아래 두 가지입니다.
